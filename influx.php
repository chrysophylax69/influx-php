...
define("INFLUX_ADDRESS_VERSION", "60");

/**
 * Influx utility functions class
 *
...
class Influx {
  ...
  private function decodeBase58($base58) {
    ...
    for ($i = 0; $i < strlen($base58); $i++) {
      $current = (string) strpos(Influx::$base58chars, $base58[$i]);
  ...
  /**
   * Convert a 160-bit Influx hash to a Influx address
   *
   * @author theymos
   * @param string $hash160
   * @param string $addressversion
   * @return string Influx address
   * @access public
   */
  public static function hash160ToAddress($hash160, $addressversion = INFLUX_ADDRESS_VERSION) {
  ...
  /**
   * Convert a Influx address to a 160-bit Influx hash
   *
   * @author theymos
   * @param string $addr
   * @return string Influx hash
   * @access public
   */
  ...
  /**
   * Determine if a string is a valid Influx address
   *
   * @author theymos
   ...
   * @access public
   */
  public static function checkAddress($addr, $addressversion = INFLUX_ADDRESS_VERSION) {
    $addr = self::decodeBase58($addr);
    if (strlen($addr) != 50) {
      return false;
    }
  ...
  /**
   * Convert the input to its 160-bit Influx hash
   *
   * @param string $data
   * @return string
   * @access private
  ...
  /**
   * Convert a Influx public key to a 160-bit Influx hash
   *
   * @param string $pubkey
   * @return string
   * @access public
...
class InfluxClientException extends ErrorException {
...
class InfluxClient extends jsonrpc_client {

  /**
   * Create a jsonrpc_client object to talk to the Influx server and return it,
   * or false on failure.
   *
   * @param string $scheme
   * "http" or "https"
   * @param string $username
   * User name to use in connection the Influx server's JSON-RPC interface
   ...
   * @access public
   * @throws InfluxClientException
   */
  public function __construct($scheme, $username, $password, $address = "localhost", $port = 5938, $certificate_path = '', $debug_level = 0) {
    $scheme = strtolower($scheme);
    if ($scheme != "http" && $scheme != "https")
      throw new InfluxClientException("Scheme must be http or https");
    if (empty($username))
      throw new InfluxClientException("Username must be non-blank");
    if (empty($password))
      throw new InfluxClientException("Password must be non-blank");
    $port = (string) $port;
    if (!$port || empty($port) || !is_numeric($port) || $port < 1 || $port > 65535 || floatval($port) != intval($port))
      throw new InfluxClientException("Port must be an integer and between 1 and 65535");
    if (!empty($certificate_path) && !is_readable($certificate_path))
      throw new InfluxClientException("Certificate file " . $certificate_path . " is not readable");
  ...
  /**
   * Test if the connection to the Influx JSON-RPC server is working
   *
  ...
  /**
   * Convert a Influx server query argument to a jsonrpcval
   *
  ...
  public function query($message) {
    if (!$message || empty($message))
      throw new InfluxClientException("Influx client query requires a message");
    ...
    if ($response->faultCode()) {
      throw new InfluxClientException($response->faultString());
    }
  ...
  public function backupwallet($destination) {
    if (!$destination || empty($destination))
      throw new InfluxClientException("backupwallet requires a destination");
   ...
   * @return float Influx balance
   * @throws InfluxClientException
   */
  public function getbalance($account = NULL, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new InfluxClientException('getbalance requires a numeric minconf >= 0');
    if ($account === NULL)
  ...
  /**
   * Returns boolean true if server is trying to generate Influxs, false otherwise.
   *
  ...
  /**
   * Tell Influx server to generate Influxs or not, and how many processors
   * to use.
   *
  ...
  public function setgenerate($generate = TRUE, $maxproc = -1) {
    if (!is_numeric($maxproc) || $maxproc < -1)
      throw new InfluxClientException('setgenerate: $maxproc must be numeric and >= -1');
  ...
  public function getaccount($address) {
    if (!$address || empty($address))
      throw new InfluxClientException("getaccount requires an address");
  ...
  public function getlabel($address) {
    if (!$address || empty($address))
      throw new InfluxClientException("getlabel requires an address");
  ...
  public function setaccount($address, $account = "") {
    if (!$address || empty($address))
      throw new InfluxClientException("setaccount requires an address");
  ...
  public function setlabel($address, $label = "") {
    if (!$address || empty($address))
      throw new InfluxClientException("setlabel requires an address");
   ...
   * @return float Influx total
   * @throws InfluxClientException
   */
  public function getreceivedbyaddress($address, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new InfluxClientException('getreceivedbyaddress requires a numeric minconf >= 0');
    if (!$address || empty($address))
      throw new InfluxClientException("getreceivedbyaddress requires an address");
  ...
  public function getreceivedbyaccount($account, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new InfluxClientException('getreceivedbyaccount requires a numeric minconf >= 0');
    if (!$account || empty($account))
      throw new InfluxClientException("getreceivedbyaccount requires an account");
  ...
  public function getreceivedbylabel($label, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new InfluxClientException('getreceivedbylabel requires a numeric minconf >= 0');
    if (!$label || empty($label))
      throw new InfluxClientException("getreceivedbylabel requires a label");
  ...
  /**
   * Return an array of arrays showing how many Influxs have been received by
  ...
  public function listreceivedbyaddress($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new InfluxClientException('listreceivedbyaddress requires a numeric minconf >= 0');
  ...
  public function listreceivedbyaccount($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new InfluxClientException('listreceivedbyaccount requires a numeric minconf >= 0');
  ...
  public function listreceivedbylabel($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new InfluxClientException('listreceivedbylabel requires a numeric minconf >= 0');
   ...
   *
   * @param string $address Destination Influx address or IP address
  ...
  public function sendtoaddress($address, $amount, $comment = NULL, $comment_to = NULL) {
    if (!$address || empty($address))
      throw new InfluxClientException("sendtoaddress requires a destination address");
    if (!$amount || empty($amount))
      throw new InfluxClientException("sendtoaddress requires an amount to send");
    if (!is_numeric($amount) || $amount <= 0)
      throw new InfluxClientException("sendtoaddress requires the amount sent to be a number > 0");
  ...
  /**
   * Check that $address looks like a proper Influx address.
   *
   * @param string $address String to test for validity as a Influx address
  ...
  public function validateaddress($address) {
    if (!$address || empty($address))
      throw new InfluxClientException("validateaddress requires a Influx address");
  ...
  public function gettransaction($txid) {
    if (!$txid || empty($txid) || strlen($txid) != 64 || !preg_match('/^[0-9a-fA-F]+$/', $txid))
      throw new InfluxClientException("gettransaction requires a valid hexadecimal transaction ID");
    return $this->query("gettransaction", $txid);
  }

  /**
   * Move Influxs between accounts.
   *
   * @param string $fromaccount
   * Account to move from. If given as an empty string ("") or NULL, Influxs will
   ...
   * Amount to move
   * @param integer $minconf
   * Minimum number of confirmations on Influxs being moved
   * @param string $comment
   * Transaction comment
   * @throws InfluxClientException
   * @since 0.3.18
  ...
  public function move($fromaccount = "", $toaccount, $amount, $minconf = 1, $comment = NULL) {
    ...
    if (!$amount || !is_numeric($amount) || $amount <= 0)
      throw new InfluxClientException("move requires a from account, to account and numeric amount > 0");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new InfluxClientException('move requires a numeric $minconf >= 0');
   ...
   * if there is less than $amount Influxs with $minconf confirmations in the
   ...
   * @param string $toaddress Influx address to send to
   * @param float $amount Amount to send
   * @param integer $minconf Minimum number of confirmations on Influxs being sent
   * @param string $comment
   * @param string $comment_to
  ...
  public function sendfrom($account, $toaddress, $amount, $minconf = 1, $comment = NULL, $comment_to = NULL) {
    if (!$account || !$toaddress || empty($toaddress) || !$amount || !is_numeric($amount) || $amount <= 0)
      throw new InfluxClientException("sendfrom requires a from account, to account and numeric amount > 0");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new InfluxClientException('sendfrom requires a numeric $minconf >= 0');
    if (!$comment && !$comment_to)
  ...
  /**
   * Return the current Influx address for receiving payments to $account.
   * The account and address will be created if $account doesn't exist.
   *
   * @param string $account Account name
   * @return string Influx address for $account
   * @throws InfluxClientException
   * @since 0.3.18
   */
  public function getaccountaddress($account) {
    if (!$account || empty($account))
      throw new InfluxClientException("getaccountaddress requires an account");
   ...
   * @param string $account
   * @return array
   * A simple array of Influx addresses associated with $account, empty
   * if the account doesn't exist.
   * @throws InfluxClientException
   */
  public function getaddressesbyaccount($account) {
    if (!$account || empty($account))
      throw new InfluxClientException("getaddressesbyaccount requires an account");
...
if (!$account) $account = "";
    ...
    if (!is_numeric($count) || $count < 0)
      throw new InfluxClientException('listtransactions requires a numeric count >= 0');
    if (!is_numeric($from) || $from < 0)
      throw new InfluxClientException('listtransactions requires a numeric from >= 0');
  ...
  public function sendmany($fromAccount, $sendTo, $minconf = 1, $comment=NULL) {
    if (!$fromAccount || empty($fromAccount))
      throw new InfluxClientException("sendmany requires an account");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new InfluxClientException('sendmany requires a numeric minconf >= 0');
